...
std::map<DisplayType_t, std::function<U8G2 *(uint8_t, uint8_t, uint8_t, uint8_t, uint8_t)>> mono_types = {
    {DisplayType_t::PCD8544, [](uint8_t reset, uint8_t clock, uint8_t data, uint8_t cs, uint8_t dc)
     { return new U8G2_PCD8544_84X48_F_4W_SW_SPI(U8G2_R2, clock, data, cs, dc, reset); }},
    {DisplayType_t::SSD1306, [](uint8_t reset, uint8_t clock, uint8_t data, uint8_t cs, uint8_t dc)
     { return new U8G2_SSD1306_128X64_NONAME_F_HW_I2C(U8G2_R0, reset, clock, data); }},
    {DisplayType_t::SH1106, [](uint8_t reset, uint8_t clock, uint8_t data, uint8_t cs, uint8_t dc)
...
void DisplayMonoClass::loop(float totalPower, float totalYieldDay, float totalYieldTotal, uint8_t isprod)
{
    ...
    snprintf(_fmtText, sizeof(_fmtText), "Solar: %5.0f W", totalYieldDay);
    printText(_fmtText, 1);

    snprintf(_fmtText, sizeof(_fmtText), "Limit: %5.0f W", totalYieldTotal);
    ...
    {
        snprintf(_fmtText, sizeof(_fmtText), " %d Inverter on", isprod);
